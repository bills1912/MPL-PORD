...
mapping_model = {
    "YOLOv8": os.path.join(absolute_path, "model", 'palm-oil-detection-collaborative.pt'),
    "YOLOv9C": os.path.join(absolute_path, "model", 'yolov9c_oil_palm_ripeness.pt'),
    "YOLOv11": os.path.join(absolute_path, "model", 'yolo11m.pt'),
    "YOLOv10": os.path.join(absolute_path, "model", 'yolov10x.pt'),
    "YOLOv8 (Pods Detection)": os.path.join(absolute_path, "model", 'pods_detection.pt'),
}

g = geocoder.ip('me')
geolocator = Nominatim(user_agent="my_geopy_app")
location = geolocator.reverse(f"{g.latlng[0]}"+","+f"{g.latlng[1]}")
        ...
        for f in d['per_frame']:
            for t in f['detections']:
                sparse.append({
                    ...
                    "detection_date": time.strftime('%A, %B %d %Y %H:%M:%S'),
                    "detection_loc": f"{address['state']}"
                })
